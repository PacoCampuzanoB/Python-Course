...
while bandera:
	...
	if opcion==1:
		...
		fecha = input("Dame la fecha, formato mm/yy: ")
		...
		cursor.execute("INSERT INTO tarjeta(numero,clave,fecha_vencimiento,cliente_id) VALUES ('%s','%s',to_date('%s','mm/yy'),%s)"%(numero,clave,fecha,num_cliente))
	
	elif opcion==3:
		cliente=input("Dame el nombre del cliente a consultar: ")
		cursor.execute("SELECT c.nombre,c.edad,c.curp,t.tarjeta_id from cliente c, tarjeta t where c.cliente_id=t.cliente_id and c.nombre='%s'"%(cliente))
